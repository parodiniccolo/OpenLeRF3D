...
"""
================================================================================================
utils for replica dataset
================================================================================================
"""


def get_replica_depth_mask(color_path, base_dir, scene_id, mask_group_name):
    mask_dir = join(base_dir, '2D_masks', scene_id, 'depth_' + mask_group_name)
    color_name = basename(color_path)
    # mask_name = 'maskraw_' + color_name.replace('jpg','png')
    mask_name = 'maskraw_' + color_name.replace('jpg', 'png')
...
def get_replica_pose(transform_path, color_path):
    with open(transform_path, 'r') as file:
        data = json.load(file)
    
    for frame in data['frames']:
        frame_filename = os.path.basename(frame['file_path'])  # Extract just the filename
        if frame_filename == color_path:
...
def get_replica_pose_transform(transform_path, color_path):
    with open(transform_path, 'r') as file:
        data = json.load(file)
    ...
    for frame in data['frames']:
        ...
        if frame_filename == color_path:
            ...
            # Multiply the specified elements by -1
            pose[0, 1] *= -1
            pose[0, 2] *= -1
            pose[1, 1] *= -1
            pose[1, 2] *= -1
...
def get_replica_depth(depth_path):
    depth = cv2.imread(depth_path, -1).astype(np.float32)
    # print(depth[depth>0])
    # depth /= 1000.
    # print(depth.shape) #(480,640)
...
def get_replica_color_and_depth_intrinsic(transform_path, color_path, downsample=1):
    
    with open(transform_path, 'r') as f:
        data = json.load(f)
    
    # Extract focal lengths and principal points
    fl_x = data["fl_x"] / downsample
    fl_y = data["fl_y"] / downsample
    cx = data["cx"] / downsample
    cy = data["cy"] / downsample
    ...
    # Since color and depth intrinsics are the same
    return intrinsic_matrix, intrinsic_matrix    


def get_replica_mask(color_path, mask_dir, scene_id, mask_group_name):    
    color_name = basename(color_path)
    mask_name = 'maskraw_' + color_name.replace('jpg', 'png')
    mask_path = join(mask_dir, mask_name)
    mask_raw = cv2.imread(mask_path, -1).astype(np.float32)
