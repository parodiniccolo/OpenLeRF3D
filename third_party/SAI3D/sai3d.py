    ...
    with open(filename, 'w') as f:
        for idx, inst_id in enumerate(insts):
            ...
            relative_output_mask_file = os.path.join(
                output_mask_path_relative, name + '_' + str(idx_extra) + '.txt')
            output_mask_file = os.path.join(
                output_mask_path, name + '_' + str(idx_extra) + '.txt')
...
# def run_sai3d(base_dir, scene_id, mask_name='sam-hq', test=False, text='', view_freq=1, thres_connect="0.9,0.5,5", 
#               dis_decay=0.5, thres_dis=0.15, thres_merge=200, max_neighbor_distance=2, similar_metric='2-norm', 
#               thres_trunc=0, from_points_thres=0.0, use_torch=False, scannetpp=False, eval_dir=""):
...
#         'text': text,
#         'view_freq': view_freq,
#         'thres_connect': np.linspace(float(thres_connect.split(',')[0]), float(thres_connect.split(',')[1]), int(thres_connect.split(',')[2])),
#         'dis_decay': dis_decay,
#         'thres_dis': thres_dis,
...
#     if scannetpp:
#         train_split, val_split = utils.get_splits(base_dir, scannetpp)

#         seg_split = val_split
#         seg_split = sorted(seg_split)
#         if scene_id is not None:
#             seg_split = scene_id.split(',')
#     else:
#         seg_split = [scene_id]

#     for scene_id in seg_split:
