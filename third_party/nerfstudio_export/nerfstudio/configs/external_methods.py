...
external_methods.append(
    ExternalMethod(
        """[bold yellow]DepthLERF[/bold yellow]
For more information visit: https://docs.nerf.studio/nerfology/methods/lerf.html

To enable LERF, you must install it first by running:
  [grey]pip install git+https://github.com/kerrj/lerf[/grey]""",
        configurations=[
            ("depthlerf-big", "DepthLERF with OpenCLIP ViT-L/14"),
            ("depthlerf", "DepthLERF with OpenCLIP ViT-B/16, used in paper"),
            ("depthlerf-lite", "DepthLERF with smaller network and less LERF samples"),
        ],
        pip_package="git+https://github.com/kerrj/lerf",
    )
)
