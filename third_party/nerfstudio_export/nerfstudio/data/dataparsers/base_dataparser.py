...
def transform_poses_to_original_space(
    ...
    # Create transformation matrix and compute inverse with fallback
    ...
    try:
        inv_transform = torch.linalg.inv(transform_matrix)
    except RuntimeError as e:
        if "cusolver" in str(e).lower():
            inv_transform = torch.linalg.inv(transform_matrix.cpu()).to(transform_matrix.device)
        else:
            raise
