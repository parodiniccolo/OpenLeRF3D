...
def generate_point_cloud(
    ...
    with progress as progress_bar:
        ...
        while not progress_bar.finished:
            ...
            rgb = rgba[mask][..., :3]
            clip = clip[mask]  # Apply mask to clip embeddings
            ...
            if crop_obb is not None:
                ...
                rgb = rgb[mask]
                view_direction = view_direction[mask]
                clip = clip[mask]  # Apply crop mask to clip embeddings
                if normal is not None:
                    normal = normal[mask]
    ...
    # Get device from points
    device = points.device
    
    # Apply the inverse transform to points
    # Create poses in the correct format (Nx3x4)
    poses = torch.zeros((points.shape[0], 3, 4), dtype=points.dtype, device=device)
    poses[..., :3, 3] = points  # Set translation
    poses[..., :3, :3] = torch.eye(3, dtype=points.dtype, device=device).unsqueeze(0).expand(points.shape[0], -1, -1)  # Set rotation

    # Ensure dataparser transform is on the same device
    pipeline.datamanager.train_dataparser_outputs.dataparser_transform = pipeline.datamanager.train_dataparser_outputs.dataparser_transform.to(device)
    ...
    # Only move to CPU when saving to HDF5
    ...
    directions_group.create_dataset("directions", data=view_directions.detach().cpu().numpy())
    rgb_group.create_dataset("rgb", data=rgbs.detach().cpu().numpy())

    # Modified CLIP data handling
    CONSOLE.print(f"CLIP embeddings shape: {clips.shape}")
    
    if len(clips.shape) == 3:  # [N, 30, 512]
        for scale in range(clips.shape[1]):
            clip_data = clips[:, scale, :]
            clip_group.create_dataset(
                f"scale_{scale}", data=clip_data.detach().cpu().numpy()
            )
    else:  # [N, 512]
        clip_group.create_dataset("embeddings", data=clips.detach().cpu().numpy())
    ...
    # Only move to CPU when creating the Open3D point cloud
    pcd = o3d.geometry.PointCloud()
    pcd.points = o3d.utility.Vector3dVector(points.detach().cpu().double().numpy())
    pcd.colors = o3d.utility.Vector3dVector(rgbs.detach().cpu().double().numpy())
    ...
    if estimate_normals:
        ...
        if ind is not None:
            normals = normals[ind]
        pcd.normals = o3d.utility.Vector3dVector(normals.detach().cpu().double().numpy())

    if reorient_normals:
        normals = torch.from_numpy(np.array(pcd.normals)).float().to(device)
        mask = torch.sum(view_directions * normals, dim=-1) > 0
        normals[mask] *= -1
        pcd.normals = o3d.utility.Vector3dVector(normals.cpu().double().numpy())
